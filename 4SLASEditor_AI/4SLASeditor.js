...
// ==================== AI CONFIG ====================
const AI_CONFIG = {
  apiUrl: 'https://satichat.ru/api/text',
  apiKey: '' // Вставьте свой API-ключ или нажмите 🤖 в редакторе
};
// Хранение ключа в localStorage
const AI_STORAGE_KEY = '4slas_ai_key';
...
class SimpleEditor {
    ...
    initToolbar() {
        ...
        toolbar.innerHTML = `
            ...
            <button type="button" data-cmd="aiMenu" title="AI-помощник">🤖 AI</button>
            <span style="margin-left:auto;font-size:.8rem;color:#b9c7e6;align-self:center;">4SLASeditor v0.3 AI</span>
        `;
    ...
    execCommand(command, value = null) {
        switch (command) {
            ...
            case 'aiMenu': this._showAiMenu(); break;
    ...
    _showMarkdownModal() {
     ...
     _mdToHtml(text) {
        ...
        // Экранируем только & и < (не > — нужен для цитат)
        html = html.replace(/&/g, '&amp;').replace(/</g, '&lt;');

        // Code blocks ```lang ... ```
        html = html.replace(/```(\w*)\s*\n([\s\S]*?)```/g, (m, lang, code) => {
            const cls = lang ? ' class="language-' + lang.replace(/[^a-zA-Z0-9]/g, '') + '"' : '';
            // Код в <pre> не экранируем (уже сделано выше)
        ...
        html = html.replace(/^\|(.+)\|\s*\n\|([-:\|\s]+)\|\s*\n((?:\|.+\|\s*\n)*)/gm, function(match, header, separator, body) {
            var out = '<table>\n<thead>\n<tr>\n';
            header.split('|').forEach(function(cell) {
                var t = cell.trim();
                if (t) out += '<th>' + t + '</th>\n';
            });
            out += '</tr>\n</thead>\n<tbody>\n';
        ...
        // Wrap paragraphs (text not inside block elements)
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (trimmed.match(new RegExp('^<(' + blocks + ')'))) {
                ...
                result += line; if (i < lines.length - 1 && lines[i+1].trim()) result += '<br>\n'; else result += '\n';
            }
        }
        if (inP) result = result.replace(/<br>\s*$/, '') + '</p>\n';
        html = result;
    ...
    _showAiMenu() {
        const existing = document.getElementById('ai-menu');
        if (existing) { existing.remove(); return; }

        this._savedSelection = (window.getSelection().toString() || '').trim();
        this._savedRange = this._getSelectedRange();
        const selText = this._savedSelection;
        const hasKey = !!AI_CONFIG.apiKey;

        const menu = document.createElement('div');
        menu.id = 'ai-menu';
        menu.className = 'ai-menu';
        const btn = this.toolbarBtns['aiMenu'];
        if (btn) {
        ...
        if (!hasKey) {
            menu.innerHTML = `
                <div class="ai-menu-item" data-ai="settings">🔑 Ввести API-ключ</div>
            `;
        } else {
            menu.innerHTML = `
                <div class="ai-menu-item ${selText ? '' : 'ai-menu-disabled'}" data-ai="grammar">📝 Проверить грамматику</div>
                <div class="ai-menu-item ${selText ? '' : 'ai-menu-disabled'}" data-ai="style">✨ Улучшить стиль</div>
                <div class="ai-menu-item ${selText ? '' : 'ai-menu-disabled'}" data-ai="rewrite">🔄 Переписать</div>
                <div class="ai-menu-sep"></div>
                <div class="ai-menu-item" data-ai="generate">💬 Спросить AI</div>
                <div class="ai-menu-sep"></div>
                <div class="ai-menu-item" data-ai="settings">⚙️ Настройки AI</div>
            `;
        }

        document.body.appendChild(menu);

        const aiBtn = this.toolbarBtns['aiMenu'];

        menu.querySelectorAll('.ai-menu-item').forEach(el => {
    ...
    _aiAction(mode) {
        const selText = this._savedSelection || window.getSelection().toString().trim() || '';
        if (selText.length === 0) {
            this._showAiToast('Сначала выделите текст в редакторе', 'error');
            return;
        }

        if (!AI_CONFIG.apiKey) {
            this._showAiToast('Сначала введите API-ключ в настройках AI', 'error');
            return;
        }
        ...
        try {
            fetch(AI_CONFIG.apiUrl + '/analyze', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-API-Key': AI_CONFIG.apiKey
                },
                body: JSON.stringify({ text: selText, mode: mode })
            })
            .then(r => {
            ...
            .then(data => {
                if (data.text) {
                    ...
                    this.syncToHidden();
                    this._updateWordCount();
                    this._showAiToast('✅ Готово', 'success');
                } else {
                    this._showAiToast('❌ Ошибка: пустой ответ', 'error');
                }
            })
            .catch(err => {
                this._showAiToast('❌ Ошибка: ' + err.message, 'error');
            })
            .finally(() => {
                if (aiBtn) aiBtn.textContent = '🤖 AI';
            });
        } catch(err) {
            this._showAiToast('❌ JS ошибка: ' + err.message, 'error');
            if (aiBtn) aiBtn.textContent = '🤖 AI';
        }
    }
    ...
    _showAiGenerate() {
        ...
        const modalId = this._nextId('ai-gen');
        const modal = document.createElement('div');
        modal.className = 'editor-modal';
        modal.style.minWidth = '450px';
        modal.innerHTML = `
            ...
            <textarea id="${modalId}-prompt" rows="5" placeholder="Напишите, что хотите сгенерировать..." style="width:100%;padding:8px;border:1px solid #2a3650;background:#0f1422;color:#e2e8f0;border-radius:4px;font-size:14px;resize:vertical;"></textarea>
            <div class="button-group" style="margin-top:12px;">
                <button id="${modalId}-ok">💬 Отправить</button>
                <button id="${modalId}-cancel" class="cancel">Отмена</button>
            </div>
        `;
        document.body.appendChild(modal);

        const promptTA = modal.querySelector(`#${modalId}-prompt`);
        promptTA.focus();
        ...
        modal.querySelector(`#${modalId}-ok`).addEventListener('click', () => {
            ...
            fetch(AI_CONFIG.apiUrl + '/generate', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-API-Key': AI_CONFIG.apiKey
                },
                body: JSON.stringify({ prompt: prompt, style: 'blog', max_length: 500 })
            })
            .then(r => {
                if (!r.ok) throw new Error('HTTP ' + r.status);
                return r.json();
    ...
    _showAiSettings() {
        const modalId = this._nextId('ai-set');
        const currentKey = AI_CONFIG.apiKey;
        const modal = document.createElement('div');
        modal.className = 'editor-modal';
        modal.style.minWidth = '400px';
        modal.innerHTML = `
            <h3>⚙️ Настройки AI-помощника</h3>
            <p style="margin-bottom:12px;color:#8a9bd5;font-size:13px;">API-ключ хранится в вашем браузере (localStorage). Никуда не отправляется.</p>
            <label>API-ключ:</label>
            <div style="display:flex;gap:6px;align-items:center;">
                <input type="password" id="${modalId}-key" value="${currentKey}" placeholder="sk-sati-..." style="flex:1;">
                <button id="${modalId}-toggle" style="cursor:pointer;background:none;border:none;color:#b9c7e6;font-size:18px;">👁</button>
            </div>
            <div class="button-group" style="margin-top:12px;">
                <button id="${modalId}-save" ${currentKey ? '' : 'disabled'}>💾 Сохранить</button>
                <button id="${modalId}-delete" class="cancel" ${currentKey ? '' : 'disabled'}>🗑 Удалить</button>
                <button id="${modalId}-cancel" class="cancel">Закрыть</button>
            </div>
        `;
        document.body.appendChild(modal);

        const keyInput = modal.querySelector(`#${modalId}-key`);
        const toggleBtn = modal.querySelector(`#${modalId}-toggle`);
        const saveBtn = modal.querySelector(`#${modalId}-save`);
        const delBtn = modal.querySelector(`#${modalId}-delete`);
        ...
        saveBtn.addEventListener('click', () => {
            ...
            modal.remove();
            this._showAiToast('✅ Ключ сохранён', 'success');
        });
        ...
        delBtn.addEventListener('click', () => {
            ...
            this._showAiToast('🗑 Ключ удалён', 'success');
            modal.remove();
        });
    ...
    // Добавить AI-стили при загрузке
    _initAiStyles() {
        ...
        style.textContent = `
            .ai-menu {
                position: fixed; z-index: 10002; background: #1e2a3e; border: 1px solid #2a3650;
                border-radius: 8px; padding: 4px 0; min-width: 220px;
                box-shadow: 0 8px 24px rgba(0,0,0,0.4);
            }
            .ai-menu-item {
                padding: 8px 16px; cursor: pointer; font-size: 14px; color: #e2e8f0;
                transition: background 0.15s; white-space: nowrap;
            }
            .ai-menu-item:hover { background: #2a3650; }
            .ai-menu-disabled { opacity: 0.4; pointer-events: none; }
            .ai-menu-sep { height: 1px; background: #2a3650; margin: 4px 0; }
            .ai-toast {
                position: fixed; bottom: 24px; right: 24px; z-index: 10003;
                padding: 10px 20px; border-radius: 6px; font-size: 14px;
                transition: opacity 0.3s; max-width: 300px;
            }
            .ai-toast--success { background: #1b5e3f; color: #fff; }
            .ai-toast--error { background: #7a2020; color: #fff; }
        `;
